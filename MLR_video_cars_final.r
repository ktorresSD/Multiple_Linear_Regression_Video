...
hist(cars_complete$mpg, main= "Histogram of MPG")
#here I a adding a vertical line where the median MPG is. I am asking R to color it blue and 4 units wide
abline(v = median(cars_complete$mpg), col = "blue", lwd = 4)

# Exploratory boxplots
boxplot(cars_complete$mpg ~ cars_complete$cylinders, col = "red", main= "MPG  by cylinders")
boxplot(cars_complete$horsepower ~ cars_complete$cylinders, col = "red", main= "Horsepower  by cylinders")
...
plot(cars_complete$horsepower,cars_complete$mpg, main= "MPG and Horsepower")

# Scatterplot by cylinder
plot(cars_complete$horsepower,cars_complete$mpg, col = cars_complete$cylinders, main= "MPG and Horsepower by Cylinders")
...
plot(subsetcars$mpg,subsetcars$horsepower, main="Simple Linear Regression",
...
plot(subsetcars$mpg,subsetcars$horsepower, main="Multiple Linear Regression \n with Interaction",
     xlab = "MPG", ylab = "Horsepower")
...
plot(subsetcars$mpg,subsetcars$horsepower, main="Comparing Simple Linear Regression and \n  Multiple Linear Regression with Interaction",
     xlab = "MPG", ylab = "Horsepower")
abline(fit.simple, col="green")
abline(fit.4, col="blue")
abline(fit.8, col="red")
...
lines(x=fit.inter$fitted.values, y=zeros, lty=1, col="red")
...
# Car with mpg = 20, cylinders=4
new.mpg4 <- data.frame(c.mpg = 20-mean(subsetcars$mpg), cylinders=4)
predict(fit.inter, new.mpg4, se.fit = TRUE)

#Car with mpg=20, cylinders=8
new.mpg8 <- data.frame(c.mpg = 20-mean(subsetcars$mpg), cylinders=8)
predict(fit.inter, new.mpg8, se.fit = TRUE)

# 95% CI and PI for car w/ mpg = 20, cylinder=4  
CI <- predict(fit.inter, new.mpg4, interval = "confidence")
CI
PI <- predict(fit.inter, new.mpg4, interval = "prediction")
PI

# 95% CI and PI for car w/ mpg = 20, cylinder=8  
CI <- predict(fit.inter, new.mpg8, interval = "confidence")
CI
PI <- predict(fit.inter, new.mpg8, interval = "prediction")
PI
